...
static int add_alias(char **args, linked_list_t *alias, alias_t *alias_struct)
{
...
alias_t *alias_struct)
{
    ...
    add_alias(args, alias, alias_struct);
    return (0);
}
...
void *data)
{
    ...
    if (len_tab(args) == 3)
        *exit_status = add_alias(args, term->alias, alias_struct);
    if (len_tab(args) > 3)
